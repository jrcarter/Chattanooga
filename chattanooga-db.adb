...
package body Chattanooga.DB is
   package User_Maps is new Ada.Containers.Hashed_Maps
      (Key_Type => Unbounded_String, Element_Type => User_Data, Hash => Ada.Strings.Unbounded.Hash, Equivalent_Keys => "=");
   ...
   function Send (From : Unbounded_String; Message : String) return Natural is
      -- Empty declarative part
   begin -- Send
      return Control.Send (From, Message);
   end Send;

   procedure Add_Friend (User : in Unbounded_String; Friend : in Unbounded_String) is
      -- Empty declarative part
   begin -- Add_Friend
      Control.Add_Friend (User => User, Friend => Friend);
   end Add_Friend;

   procedure Remove_Friend (User : in Unbounded_String; Friend : in Unbounded_String) is
      -- Empty declarative part
   begin -- Remove_Friend
